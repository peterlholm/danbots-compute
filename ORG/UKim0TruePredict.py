...
def getnnplys(infolder ):       
    count=len(os.listdir(infolder))-3
    for i in range(count):
        print(count)
        shutil.copyfile(infolder+'/render'+str(i)+'/pointcl-nndepth.ply', infolder +'files/nnplyfolder/points'+str(i)+'.ply')
        # shutil.copyfile(infolder+'/render'+str(i)+'/depth.png', infolder +'files/depthfolder/image'+str(i)+'.png')  
...
def load_H_model():
    ...
    model = tensorflow.keras.models.load_model('/home/samir/dbI/models/nnmat/nn1matsalm1.h5')
    return(model)
# /home/samir/dblive/cnnpredict/models/cnnres01-220-modelwrap1'+'-200-adam-noBN.h5

def load_L_model():
    model = tensorflow.keras.models.load_model('/home/samir/dbI/models/nnmat/nn2salkmodel1.h5')#    wr1uwrpl-slim-ES-cscans.h5')
    # model = tensorflow.keras.models.load_model('/home/samir/MSThesis/004_intermediate_output/20211011/09h41m04s/model.h5')
    return(model)
...
def mask(folder):
    ...
    cv2.imwrite( folder+ 'mask.png', 255*mask)
    return(mask)
...
def stretchfile(img):
    ...
    # if not(exists(folder+'image0s.png' )):
    # shutil.copy(folder+ '/image0.png' , folder+ '/image0s.png' )
    # img= cv2.imread(folder+ '/image0.png',0)
...
def nnHprocess(folder):
    ...
    # image1 = stretchfile(image1)
    # p2, p98 = np.percentile(image1, (2, 98))
    # image1 = exposure.rescale_intensity(image1, in_range=(p2, p98))
    ...
    cv2.imwrite( folder + 'nnwrap1.png',255*predicted_img)
...
def nnLprocess(folder):
    ...
    np.save(folder + 'nnkdata.npy', 1*nnkdata, allow_pickle=False)
    # prdicted_img = np.round(predicted_img*17/(np.max(predicted_img)))
    cv2.imwrite( folder + 'nnkdata.png',1*nnkdata)
...
#     end = time.time()
#     print('elapsed low:', end-start)
#     np.save( 'kdata.npy', predicted_img, allow_pickle=False)
#     # cv2.imwrite(folder+'kdata.png',predicted_img )
#     # mask = np.load(folder+'mask.npy')
#     # predicted_img = np.multiply(np.logical_not(mask), predicted_img)


#     nnunwrap = np.add(inp_1*2*PI, np.multiply(2*PI,predicted_img) )
#     # nnunwrap= (1*predicted_img)
#     # nnunwrap= 2*PI*predicted_img
#     np.save(folder + 'nnunwrap.npy', nnunwrap, allow_pickle=False)
#     # prdicted_img = np.round(predicted_img*17/(np.max(predicted_img)))
#     cv2.imwrite( folder + 'nnunwrap.png',nnunwrap)
#     # print('255*kdata:', 255*kdata[::40, ::40])
...
def unwrap_r(low_f_file, high_f_file, folder):
    ...
    cv2.imwrite(folder + 'nnunwrap.png', im_unwrap)
    cv2.imwrite(folder + 'nnkdata.png', np.multiply(2*PI,kdata))
...
def newDepth(folder, basecount):
    ...
    DBase = np.load(basefile)
    unwrap = np.load(folder+'nnkunwrap.npy' )/1 #2.5
    mask = np.load(folder+'mask.npy' )
    # print('DBase:', np.amax(DBase), np.amin(DBase))
    # print('unwrap:', np.amax(unwrap), np.amin(unwrap))
...
def unw(myfolder):
    unwrap_r('nnwrap2.npy', 'nnwrap1.npy', myfolder )
...
def makeclouds(scanfolder, count):
     for i in range(count):
        ...
        if path.exists(folder):
            ...
            generate_pointcloud(folder + 'image8.png', folder + 'mask.png', folder + 'nndepth.png', folder +'pointcl-nndepth.ply')
...
# folder = '/home/samir/Desktop/blender/pycode/bldev2/heads/render'
# bfolder = '/home/samir/Desktop/blender/pycode/bldev2/heads/'
folder = '/home/samir/Desktop/cal/render'
bfolder = '/home/samir/Desktop/cal'
...
for i in range (11):# (00,len(os.listdir(bfolder))):
    i = i+offset
    ...
    # unw(folder + str(i)+'/')
    # start=time.time()
    newDepth(folder+ str(i)+'/' , 30)
    # end= time.time()
    # print('depth calc time =', end-start)
